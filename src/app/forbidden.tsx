 ...
 * Forbidden Page (403)
 *
 * Renders the CMS "/forbidden" system page if available, otherwise
...
export default async function Forbidden() {
  ...
  try {
    ...
    const resolution = await resolvePath("/forbidden", locale);
    if (resolution.type === "page" && resolution.content) {
      const content = parsePageContent(resolution.content);
      sections = content?.sections ?? [];
    }
  } catch {
    // CMS forbidden page not available -- use fallback
  }
